...
print('Linear Regression test_error =', run_linear_regression_on_MNIST(lambda_factor=0.001))
...
def run_softmax_on_MNIST(temp_parameter=1):
    ...
    print('error rate of new labels using mod3 = ',test_error_mod3)
    return test_error
...
print('T = 2   softmax test_error=', run_softmax_on_MNIST(temp_parameter=2))
'''
...
def run_softmax_on_MNIST_mod3(temp_parameter=1):
    ...
    # Save the model parameters theta obtained from calling softmax_regression to disk.
    write_pickle_data(theta, "./theta_onmod3.pkl.gz")
    return test_error
'''
...
write_pickle_data(theta, "./theta_onPCA.pkl.gz")
...
write_pickle_data(theta, "./theta_onCubic.pkl.gz")

print('softmax Cubic Feature test_error=', test_error)
'''
'''
pred_test_y = cubic_svm(train_pca10, train_y, test_pca10)
test_error = compute_test_error_svm(test_y, pred_test_y)
print('Cubic SVM test_error:', test_error)

pred_test_y = rbf_svm(train_pca10, train_y, test_pca10)
test_error = compute_test_error_svm(test_y, pred_test_y)
print('Cubic SVM test_error:', test_error)
'''

def run_kernel_softmax_on_MNIST(kernel_train, train_y, kernel_test, test_y, temp_parameter= 1.0, lambda_factor=1.0e-4, k=10, learning_rate=0.3, num_iterations=150):
    ...
    plot_cost_function_over_time(cost_function_history)
    test_error_kernel = kernel_test_error(alphas, kernel_test, test_y, temp_parameter=1)
